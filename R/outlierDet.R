...
function(dat.all,samp.names,HPC_mode=F){
  ...
  if (HPC_mode==T){
  	bitmap(paste(Path,DF,paste(DF,"Positive_Mode","outler_detection.bmp",sep="_"),sep="/"))
  } else {
  	jpeg(paste(Path,DF,paste(DF,"Positive_Mode","outler_detection.jpg",sep="_"),sep="/"))
  } 
  ...
  if (HPC_mode==T){
  	...
  	jpeg(paste(Path,DF,paste(DF,"Negative_Mode","outlier_detection.jpg",sep="_"),sep="/"))
  }
