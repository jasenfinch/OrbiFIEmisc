...
function(x,samp.names, method = "classical", conf.level = 0.975, dimen=c(1,2), tol = 1e-7, plotting = TRUE){ 
  ...
  plot.elli <- function(x,samp.names, cov,center,dimen,conf.level=0.95, dis, cutoff){
    ...
    box()
    outlier.label(x,samp.names,dis,cutoff)   ## label outliers
  ...
  outlier.label <- function(x,samp.names, dis, cutoff){
  ...
  outlier <- samp.names[which(dis > cutoff)]

  ## plot robust mahalanobis distances
  if (plotting){
    par(mfrow=c(1,2), pty="m")
    plot.dist(dis,samp.names, cutoff)                  
    plot.elli(x,samp.names, rob$cov, rob$center,dimen=dimen,conf.level=conf.level, dis, cutoff)
  }
